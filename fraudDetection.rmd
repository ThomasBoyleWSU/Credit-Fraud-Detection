...
Test tuned xgBoost
```{r}
xgBoostTuned = readRDS("xgBoostTuned.rds")

prediction = predict(xgBoostTuned, as.matrix(data[-trainIndeces,-31]))

pr <- pr.curve(scores.class0 = prediction, weights.class0 = as.numeric(data[-trainIndeces, "Class"]), curve = T)
plot(pr, main = "Tuned XGBoost PR Curve")
roc <- roc.curve(scores.class0 = prediction, weights.class0 = as.numeric(data[-trainIndeces, "Class"]), curve = T)
plot(roc)

print("Actual as rows - Predicted as columns")
print("Prediction greater than 10%")
table(ifelse(data[-trainIndeces, "Class"] == 1, "Fraud", "Legitimate"), ifelse(prediction >= 0.1, "Fraud", "Legitimate"))
print("Prediction greater than 30%")
table(ifelse(data[-trainIndeces, "Class"] == 1, "Fraud", "Legitimate"), ifelse(prediction >= 0.3, "Fraud", "Legitimate"))
print("Prediction greater than 50%")
table(ifelse(data[-trainIndeces, "Class"] == 1, "Fraud", "Legitimate"), ifelse(prediction >= 0.5, "Fraud", "Legitimate"))
```

Test untuned xgBoost
```{r}
xgBoostUntuned = readRDS("xgBoostUntuned.rds")
...
table(as.numeric(data[-trainIndeces, "Class"]), predicted)
```

Test random forest
```{r}
RFModel = readRDS("RandomForest.rds")

prediction = predict(RFModel, as.matrix(data[-trainIndeces,-31]), type = "prob")

pr <- pr.curve(scores.class0 = prediction[,2], weights.class0 = as.numeric(data[-trainIndeces, "Class"]), curve = T)
plot(pr, main = "Random Forest PR Curve")
roc <- roc.curve(scores.class0 = prediction[,2], weights.class0 = as.numeric(data[-trainIndeces, "Class"]), curve = T)
plot(roc)
library(ROCR)
predict = ROCR::prediction(prediction[,2], data[-trainIndeces, "Class"])
perf = performance(predict, "prec", "rec")
plot(perf, colorize = T)
RFModel$confusion

predicted = ifelse(prediction[,2] >= 0.3, 1, 0)
table(as.numeric(data[-trainIndeces, "Class"]), predicted)
```
...
```{r}
gc()
RFModel = randomForest(Class ~ ., data = data, subset = trainIndeces)
saveRDS(rfModel, "RandomForest.rds")
```
...
saveRDS(xgBoostUntuned, "xgBoostUntuned.rds")
```
...
saveRDS(xgBoostTuned, "xgBoostTuned.rds")
```
